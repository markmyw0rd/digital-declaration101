...
export async function POST(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const parties = await sql`SELECT role, email, signed_at, signature_blob_url 
                            FROM envelope_parties WHERE envelope_id=${params.id} ORDER BY role`;
  ...
  await sql`INSERT INTO files (envelope_id,type,url,sha256) 
