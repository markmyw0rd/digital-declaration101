...
export async function POST(req: NextRequest){
  ...
  const unit = await sql`INSERT INTO units (code,name) VALUES (${unitCode},${unitName||unitCode})
    ON CONFLICT (code) DO UPDATE SET name=EXCLUDED.name RETURNING id`;
  const student = await sql`INSERT INTO users (email,name,role) VALUES (${studentEmail},${studentName||""},'student')
    ON CONFLICT (email) DO UPDATE SET name=EXCLUDED.name RETURNING id`;
  const env = await sql`INSERT INTO envelopes (unit_id, student_id, status)
    VALUES (${unit.rows[0].id}, ${student.rows[0].id}, 'awaiting_student') RETURNING id`;
  await sql`INSERT INTO envelope_parties (envelope_id, role, email) VALUES
